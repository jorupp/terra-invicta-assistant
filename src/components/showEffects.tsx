...
export const ShowEffects = (props: ShowEffectsProps & { highlightMissionClassName?: (missionName: MissionDataName) => string | undefined, highlightTier?: boolean }) => {
  ...
  const persuasion = Math.max(0, (props.persuasion || 0) + (props.Persuasion || 0));
  const command = Math.max(0, (props.command || 0) + (props.Command || 0));
  const investigation = Math.max(0, (props.investigation || 0) + (props.Investigation || 0));
  const espionage = Math.max(0, (props.espionage || 0) + (props.Espionage || 0));
  const administration = Math.max(0, (props.administration || 0) + (props.Administration || 0));
  const science = Math.max(0, (props.science || 0) + (props.Science || 0));
  const security = Math.max(0, (props.security || 0) + (props.Security || 0));
