...
const buildIcon = (name: string, src: string, showLightBg?: boolean) => {
  ...
  const Icon = (props: ComponentProps<"img"> & { iconClassName?: string }) => (
    <span className={twMerge('inline-block rounded radius-4 p-0.5 -m-0.5 -my-1.5', extraClassName, props.className)} title={name}>
      <img src={src} {...props} className={twMerge("h-4 w-4 max-w-4", props.iconClassName)} />
    </span>
  );
...
export const TraitGovernment = (props: ComponentProps<"span"> & { strokeClass?: string }) => 
  <span className={twMerge('inline-block rounded radius-4 p-0.5 -m-0.5 -my-1.5', props.className)}>
    <Building2 className={twMerge("h-4 w-4", props.strokeClass || "stroke-green-500")} />
  </span>;
...
export const Miltech = buildIcon("Miltech", "https://wiki.hoodedhorse.com/images/mbhh_ti/d/df/ICO_army_level.png");
export const HabPower = buildIcon("Power", "https://wiki.hoodedhorse.com/images/mbhh_ti/4/4f/ICO_hab_power.png", true);
