...
interface HabEntity extends NamedEntity {
  site?: { parentBodyId: number } | null;
  orbitStateId?: number;
}
...
interface FactionGoals {
  CaptureNationClean?: { value: number }[];
  CaptureNationDirty?: { value: number }[];
  NeutralizeNation?: { value: number }[];
  AttackWithFleet?: { value: number }[];
  DefendWithFleet?: { value: number }[];
  WarOnFaction?: { value: number }[];
  InvadeEarth?: { value: number }[];
  BuildFullStation?: { value: number }[];
  BuildFullBase?: { value: number }[];
}

export function expandAlienGoals(
  saveFile: SaveFile,
  factionGoals: FactionGoals | undefined,
  nationsById: Map<number, NamedEntity>,
  habs: HabEntity[],
  fleets: NamedEntity[],
  factionsById: Map<number, NamedEntity>,
  bodiesById: Map<number, BodyEntity>,
  orbitsById: Map<number, OrbitEntity>,
): ExpandedGoal[] {
  ...
  if (factionGoals) {
    // CaptureNationClean
    factionGoals.CaptureNationClean?.forEach((goalRef) => {
    ...
    // CaptureNationDirty
    factionGoals.CaptureNationDirty?.forEach((goalRef) => {
    ...
    // NeutralizeNation
    factionGoals.NeutralizeNation?.forEach((goalRef) => {
    ...
    // AttackWithFleet
    factionGoals.AttackWithFleet?.forEach((goalRef) => {
    ...
    // DefendWithFleet
    factionGoals.DefendWithFleet?.forEach((goalRef) => {
    ...
    // WarOnFaction
    factionGoals.WarOnFaction?.forEach((goalRef) => {
    ...
    // InvadeEarth
    factionGoals.InvadeEarth?.forEach((goalRef) => {
    ...
    // BuildFullStation
    factionGoals.BuildFullStation?.forEach((goalRef) => {
    ...
    // BuildFullBase
    factionGoals.BuildFullBase?.forEach((goalRef) => {
