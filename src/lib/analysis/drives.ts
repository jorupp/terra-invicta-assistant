...
interface PlayerFactionForDrives {
...
interface GlobalTechStateForDrives {
...
export async function analyzeDrives(
  playerFaction: PlayerFactionForDrives,
  globalTechState: GlobalTechStateForDrives,
  techs: Map<string, { prereqs?: string[]; researchCost: number }>,
  projects: Map<string, { prereqs?: string[]; researchCost: number; factionAvailableChance?: number }>,
  driveLocalization: Map<string, string>,
  powerPlantLocalization: Map<string, string>,
) {
  ...
  const drives = Array.from(drivesByBaseName.values()).map((drive) => {
    const { techResearchRemaining, projectResearchRemaining, requiredTechs, requiredProjects } =
      calculateRemainingResearch(drive.requiredProjectName, globalTechState, playerFaction, techs, projects);
