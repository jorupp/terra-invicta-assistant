...
function CurrentHabBonusesSection({ analysis }: { analysis: Analysis }) {
...
function FutureHabBonusesSection({ analysis }: { analysis: Analysis }) {
...
function MCBoostIncomeSummarySection({ analysis }: { analysis: Analysis }) {
  const { nationHistory } = analysis.playerFaction;
  return (
    <div className="flex space-x-4 pb-4">
      <span>
        <Boost />
        {nationHistory.currentBoost.toFixed(2)}
        {nationHistory.boostMonthlyChange !== 0 && (
          <span className={nationHistory.boostMonthlyChange > 0 ? "text-green-600" : "text-red-600"}>
            {" "}({nationHistory.boostMonthlyChange > 0 ? "+" : ""}
            {nationHistory.boostMonthlyChange.toFixed(2)})
          </span>
        )}
      </span>
      <span>
        <MissionControl />
        {nationHistory.currentMC.toFixed(0)}
        {nationHistory.mcMonthlyChange !== 0 && (
          <span className={nationHistory.mcMonthlyChange > 0 ? "text-green-600" : "text-red-600"}>
            {" "}({nationHistory.mcMonthlyChange > 0 ? "+" : ""}
            {nationHistory.mcMonthlyChange.toFixed(0)})
          </span>
        )}
      </span>
    </div>
...
function AvailableExpandNationSection({ analysis }: { analysis: Analysis }) {
  ...
  return (
    <div className="space-y-2">
      <Card>
        ...
        <CardContent>
          <div className="flex gap-4 text-sm">
            {(["water", "volatiles", "metals", "nobles", "fissiles"] as const).map((res) => {
              const IconMap = { water: Water, volatiles: Volatiles, metals: Metals, nobles: Nobles, fissiles: Fissiles };
              const Icon = IconMap[res];
              return (
                <button key={res} className="flex items-center gap-1 cursor-pointer hover:underline" onClick={() => handleMineResourceSort(res)}>
                  <Icon />
                  {((analysis.playerFaction.miningMultipliers[res] - 1) * 100).toFixed(0)}%
                </button>
              );
            })}
          </div>
...
export function getHabsSections(analysis: Analysis) {
  const { playerHabs } = analysis;
  const {
    availableBoostProjects,
    availableCPProjects,
    availableMaxOrgProjects,
    availableExpandNationProjects,
  } = analysis.playerFaction;
  const { playerStealableProjects } = analysis;

  const missingMines = playerHabs.filter((h) => h.missingMine);
  const nextCompletion = playerHabs
    .flatMap((i) => i.highlightedCompletions)
    .filter((i) => i)
    .toSorted((a, b) => a.daysToCompletion - b.daysToCompletion)[0];

  const subtitle = [
    `${playerHabs.length} habs`,
    nextCompletion ? `next: ${nextCompletion.daysToCompletion.toFixed(0)}d` : null,
    missingMines.length > 0 ? `${missingMines.length} missing mine(s)` : null,
  ]
    .filter(Boolean)
    .join(", ");

  const conditionalLeaves = [
    availableBoostProjects.length > 0 && { id: "habs.available-boost-projects", label: "Available Boost Projects" },
    availableCPProjects.length > 0 && { id: "habs.available-cp-projects", label: "Available CP Projects" },
    availableMaxOrgProjects.length > 0 && { id: "habs.available-max-org-projects", label: "Available Max Org Projects" },
    availableExpandNationProjects.length > 0 && { id: "habs.available-expand-nation-projects", label: "Available Expand Nations" },
    playerStealableProjects.length > 0 && { id: "habs.available-stealable-projects", label: "Available Stealable Projects" },
  ].filter(Boolean) as { id: string; label: string }[];

  const conditionalContents: [string, React.ReactNode][] = [
    availableBoostProjects.length > 0 && ["habs.available-boost-projects", <AvailableBoostProjectsSection key="boost-proj" analysis={analysis} />],
    availableCPProjects.length > 0 && ["habs.available-cp-projects", <AvailableCPProjectsSection key="cp-proj" analysis={analysis} />],
    availableMaxOrgProjects.length > 0 && ["habs.available-max-org-projects", <AvailableMaxOrgProjectsSection key="max-org" analysis={analysis} />],
    availableExpandNationProjects.length > 0 && ["habs.available-expand-nation-projects", <AvailableExpandNationSection key="expand" analysis={analysis} />],
    playerStealableProjects.length > 0 && ["habs.available-stealable-projects", <AvailableStealableProjectsSection key="stealable" analysis={analysis} />],
  ...
  return {
    group: {
      ...
      subtitle,
      leaves: [
        { id: "habs.current-bonuses", label: "Current Hab Bonuses" },
        { id: "habs.future-bonuses", label: "Future Hab Bonuses" },
        { id: "habs.boost-mc-summary", label: "MC/Boost Income Summary" },
        { id: "habs.alien-hate", label: "Alien Hate" },
        { id: "habs.building-details", label: "Building Details" },
        ...conditionalLeaves,
        { id: "habs.technology-goals", label: "Technology Goals" },
        { id: "habs.manage-habs", label: "Manage Habs" },
        { id: "habs.manage-mines", label: "Manage Mines" },
      ],
    },
    contents: new Map<string, React.ReactNode>([
      ["habs.current-bonuses", <CurrentHabBonusesSection key="cur-bon" analysis={analysis} />],
      ["habs.future-bonuses", <FutureHabBonusesSection key="fut-bon" analysis={analysis} />],
      ["habs.boost-mc-summary", <MCBoostIncomeSummarySection key="mc-boost" analysis={analysis} />],
      ["habs.alien-hate", <AlienHateSection key="alien-hate" analysis={analysis} />],
      ["habs.building-details", <BuildingDetailsSection key="buildings" analysis={analysis} />],
      ...conditionalContents,
      ["habs.technology-goals", <TechnologyGoalsSection key="tech-goals" analysis={analysis} />],
      ["habs.manage-habs", <ManageHabsSection key="manage-habs" analysis={analysis} />],
      ["habs.manage-mines", <ManageMinesSection key="manage-mines" analysis={analysis} />],
    ]),
  };
}
