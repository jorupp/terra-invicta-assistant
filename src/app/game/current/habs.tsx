...
function HabsManageMinesView({ analysis }: { analysis: Analysis }) {
  ...
  return (
    <div className="space-y-2">
      <Card><CardHeader><CardTitle>Mining Bonuses</CardTitle></CardHeader><CardContent><div className="flex gap-4 text-sm">{["water", "volatiles", "metals", "nobles", "fissiles"].map((r) => { const Icon = ({ water: Water, volatiles: Volatiles, metals: Metals, nobles: Nobles, fissiles: Fissiles } as Record<string, typeof Water>)[r] || Water; const mult = analysis.playerFaction.miningMultipliers[r as keyof typeof analysis.playerFaction.miningMultipliers] || 1; const bonus = ((mult - 1) * 100).toFixed(0); return <button key={r} className="flex items-center gap-1 cursor-pointer hover:underline" onClick={() => handleMineResourceSort(r as MineResourceType)}><Icon />{bonus}%</button>; })}</div></CardContent></Card>
