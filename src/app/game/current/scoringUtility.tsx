import { MissionDataName, TechCategory, TraitDataName } from "@/lib/template-types-generated";
import { ShowEffectsProps } from "@/components/showEffects";
...
export function getBaseCouncilorScore(
  councilor: Parameters<typeof getScore>[0] & { effectsBaseAndUnaugmentedTraits: ShowEffectsProps },
  weights: ScoringWeights,
  haveMissions: Map<MissionDataName, number>
): ScoreResult {
...
export function getModifiedCouncilorScore(
  councilor: Parameters<typeof getScore>[0] & { effectsWithOrgsAndAugments: ShowEffectsProps },
  weights: ScoringWeights,
  haveMissions: Map<MissionDataName, number>
): ScoreResult {
...
function getScore(
  ...
  const addScore = (name: string, value: number | undefined, weight: number | undefined, noNegative?: boolean) => {
    ...
    details.push(
      `${name}: ${parseFloat(actualValue.toFixed(2))} x ${parseFloat(actualWeight.toFixed(3))} = ${contribution.toFixed(
        3
      )}`
    );
  };
