  ...
  return (
    <div className="space-y-2">
      <Accordion type="single" collapsible defaultValue="transactions">
        ...
        <AccordionItem value="nation-history">
          <AccordionTrigger>Nation History (MC & Boost)</AccordionTrigger>
          <AccordionContent>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {(() => {
                  const { historyBoost, historyMissionControl } = analysis.playerFaction.nationHistory;
                  const maxLength = Math.max(historyBoost.length, historyMissionControl.length);

                  return Array.from({ length: maxLength }, (_, index) => {
                    const boost = historyBoost[index];
                    const boostPrev = index > 0 ? historyBoost[index - 1] : undefined;
                    const boostDiff = boostPrev !== undefined && boost !== undefined ? boostPrev - boost : undefined;

                    const mc = historyMissionControl[index];
                    const mcPrev = index > 0 ? historyMissionControl[index - 1] : undefined;
                    const mcDiff = mcPrev !== undefined && mc !== undefined ? mcPrev - mc : undefined;

                    return (
                      <TableRow key={index}>
                        <TableCell className="text-right">{boost !== undefined ? boost.toFixed(2) : "-"}</TableCell>
                        <TableCell className="text-right">
                          {boostDiff !== undefined ? (
                            <span className={boostDiff > 0 ? "text-green-600" : boostDiff < 0 ? "text-red-600" : ""}>
                              {boostDiff > 0 ? "+" : ""}
                              {smartRound(boostDiff)}
                            </span>
                          ) : (
                            "-"
                          )}
                        </TableCell>
                        <TableCell className="text-right">{mc !== undefined ? smartRound(mc) : "-"}</TableCell>
                        <TableCell className="text-right">
                          {mcDiff !== undefined ? (
                            <span className={mcDiff > 0 ? "text-green-600" : mcDiff < 0 ? "text-red-600" : ""}>
                              {mcDiff > 0 ? "+" : ""}
                              {smartRound(mcDiff)}
                            </span>
                          ) : (
