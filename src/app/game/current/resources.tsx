...
function TransactionsSection({ analysis }: { analysis: Analysis }) {
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <div className="space-y-1">
                            {data.transactions.map((txn, i) => (
                              <div key={i}>
                                {txn.date}: {smartRound(txn.amount)}
                              </div>
                            ))}
                          </div>
                        </TooltipContent>
...
function OwnedNationsSection({ analysis }: { analysis: Analysis }) {
  const {
    nations,
    factionsById,
    playerFaction: { permaAbandonedNationIds, id: playerFactionId },
        ...
        {nations
          ...
          .map((nation) => (
            <TableRow key={nation.id} className={getNationBg(nation)}>
              ...
              <TableCell>
                {nation.allocatedPriorities.LaunchFacilities ? pct(nation.allocatedPriorities.LaunchFacilities) : null}
              </TableCell>
              <TableCell>
                {nation.allocatedPriorities.MissionControl ? pct(nation.allocatedPriorities.MissionControl) : null}
              </TableCell>
              <TableCell>
                {nation.allocatedPriorities.Spoils ? pct(nation.allocatedPriorities.Spoils) : null}
              </TableCell>
...
function SpoilTargetsSection({ analysis }: { analysis: Analysis }) {
...
function MCBoostTargetsSection({ analysis }: { analysis: Analysis }) {
  const { nations } = analysis;
  return (
    <Table>
      <TableHeader>
...
export function getResourcesSections(analysis: Analysis) {
  const spoils = analysis.playerFaction.monthlyTransactionSummary
    .filter((i) => i.resource === "Money" && i.source === "Spoils")
    .reduce((sum, i) => sum + i.amount, 0);
  const { mcUsage, mcCurrentLimit } = analysis.playerFaction;

  return {
    group: {
      id: "resources",
      label: "Resources",
      subtitle: `$${spoils.toFixed(0)} spoils, MC ${mcUsage.toFixed(0)}/${mcCurrentLimit.toFixed(0)}`,
      leaves: [
        { id: "resources.transactions", label: "Transactions" },
        { id: "resources.owned", label: "Owned Nations" },
        { id: "resources.spoils", label: "Spoil Targets" },
        { id: "resources.space", label: "MC/Boost Targets" },
        { id: "resources.nation-claims", label: "Nation Claims" },
        {
          id: "resources.unification-candidates",
          label: `Unification Candidates (${analysis.unificationCandidates.length})`,
        },
      ],
    },
    contents: new Map([
      ["resources.transactions", <TransactionsSection key="tx" analysis={analysis} />],
      ["resources.owned", <OwnedNationsSection key="owned" analysis={analysis} />],
      ["resources.spoils", <SpoilTargetsSection key="spoils" analysis={analysis} />],
      ["resources.space", <MCBoostTargetsSection key="space" analysis={analysis} />],
      ["resources.nation-claims", <NationClaimsSection key="claims" analysis={analysis} />],
      [
        "resources.unification-candidates",
        <UnificationCandidatesSection key="unif" analysis={analysis} />,
      ],
    ]),
  };
}
