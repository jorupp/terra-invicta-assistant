...
function makeHighlightFns(playerMissionCounts: Map<MissionDataName, number>) {
  return {
    currentHighlight(missionName: MissionDataName) {
      const count = playerMissionCounts.get(missionName) || 0;
      if (count === 2) return "bg-yellow-300/50";
      if (count === 1) return "bg-red-300/50";
    },
    availableHighlight(missionName: MissionDataName) {
      const count = playerMissionCounts.get(missionName) || 0;
...
function ExistingCouncilSection({
  analysis,
  weights,
  setWeights,
  ...
  const { currentHighlight, availableHighlight } = makeHighlightFns(analysis.playerMissionCounts);
  return (
        ...
        <TableBody>
          {scoredModifiedCouncilors.map((councilor) => (
            <CouncilorTableRow
              ...
              hasOrgs
              highlightMissionClassName={currentHighlight}
            />
          ))}
        </TableBody>
      </Table>
        ...
        <TableBody>
          {scoredOrgs.map((org) => (
            <OrgTableRow
              ...
              highlightMissionClassName={availableHighlight}
            />
          ))}
        </TableBody>
      </Table>
...
function FindNewCouncilorsSection({
  analysis,
  ...
  const { availableHighlight, currentHighlight } = makeHighlightFns(analysis.playerMissionCounts);
  return (
...
function CurrentOrgsSection({
  analysis,
  scoredOwnedOrgs,
}: {
  analysis: Analysis;
  scoredOwnedOrgs: (Analysis["playerAvailableOrgs"][number] & { type: string; score: ScoreResult; councilor?: string; councilorId?: number })[];
}) {
  const playerNationIds = new Set(analysis.playerNationIds);
  const playerTraits = new Set(analysis.playerCouncilors.flatMap((i) => i.traitTemplateNames));
  const { availableHighlight } = makeHighlightFns(analysis.playerMissionCounts);
  return (
    <Table>
...
function HostileTakeoverSection({
  analysis,
  weights,
}: {
  analysis: Analysis;
...
function MissionsSection({ analysis }: { analysis: Analysis }) {
  const {
    playerMissionCounts,
  ...
  const playerNationIds = new Set(analysis.playerNationIds);
  const playerTraits = new Set(playerCouncilors.flatMap((i) => i.traitTemplateNames));
  ...
  const sourcesByFactionByMission = missionSources
    .reduce((acc, o) => {
      ...
      {factions.map((faction) => (
        <TabsContent key={faction.id} value={`faction-${faction.id}`}>
          <SmartAccordion type="single" collapsible storageKey={`councilorsMissions-${faction.id}`}>
            {Array.from(new Set([...importantMissions, ...Array.from(sourcesByFactionByMission.get(faction.id)?.keys() || [])])).map(
              (missionName) => {
                ...
                return (
                  <AccordionItem key={missionName} value={missionName}>
                    <AccordionTrigger>
                      <span>
                        <MissionIcon /> {missionName} &ndash; {sources.filter((i) => i.type === "councilor").length} Councilors &lt;-{" "}
...
export function getCouncilorsSections(
  analysis: Analysis,
  weights: ScoringWeights,
  setWeights: (w: ScoringWeights) => void
) {
  ...
  return {
    group: {
      id: "councilors",
      label: "Councilors",
      subtitle: `${worstExistingCouncilor?.toFixed(0)} vs ${bestAvailableCouncilor?.toFixed(0)} / Orgs ${worstExistingOrg?.toFixed(2)} vs ${bestAvailableOrg?.toFixed(2)}`,
      leaves: [
        { id: "councilors.existing", label: "Existing Council" },
        { id: "councilors.find-new", label: "Find New Councilors" },
        { id: "councilors.current-orgs", label: "Current Organizations" },
        { id: "councilors.takeover", label: "Hostile Takeover" },
        { id: "councilors.missions", label: "Missions" },
        { id: "councilors.other-councilors", label: "Other Councilors" },
      ],
    },
    contents: new Map([
      [
        "councilors.existing",
        <ExistingCouncilSection
          key="existing"
          analysis={analysis}
          weights={weights}
      ...
      [
        "councilors.current-orgs",
        <CurrentOrgsSection key="current-orgs" analysis={analysis} scoredOwnedOrgs={scoredOwnedOrgs} />,
      ],
      [
        "councilors.takeover",
        <HostileTakeoverSection key="takeover" analysis={analysis} weights={weights} />,
      ],
      ["councilors.missions", <MissionsSection key="missions" analysis={analysis} />],
      ["councilors.other-councilors", <OtherCouncilorsByFaction key="other" analysis={analysis} weights={weights} />],
    ]),
  };
}
